...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #JXCoinTEST2\r");
            Send(hSocket, "WHO #JXCoinTEST2\r");
        } else {
            // randomly join #JXCoin00-#JXCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #JXCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #JXCoin%02d\r", channel_number).c_str());
        }
